...
//     	window.alert("test1");
    	...
    	$.post("ajaxdemo",{ "user": ($("#username").val()) },function(data){
// 			alert("反馈信息："+date);
// 			alert(typeof(eval(data)));
			var treeData = data.resultTree; 
			treeData = eval('('+data+')');
			//遍历JSON对象及其属性值
			for(var i in treeData){
				//alert(treeData[i].name);
				for(var j in treeData[i]){
					alert("treeData["+i+"]["+j+"]:"+treeData[i][j]);
				}
				//$("#ris").html(i.name);
			}
			$("#ris").html(data);
